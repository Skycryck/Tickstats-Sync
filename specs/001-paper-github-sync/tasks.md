...
- [X] T007 [P] Implement `PatMasker` in `src/main/java/com/skycryck/tickstatssync/util/PatMasker.java` — single `volatile String currentToken`, `setToken(String)`, and `String mask(String)` that performs the literal `String.replace(currentToken, "***")` contract from [research.md](research.md) §R8; no-op when token is null or empty.
...
- [X] T009 [P] Author `PatMaskerTest` in `src/test/java/com/skycryck/tickstatssync/util/PatMaskerTest.java` — property-style coverage: token value never survives `mask()` across hundreds of randomized substrings; no-op before `setToken`; rotation swaps the redacted value with no leakage window.
- [X] T010 [P] Author `LogRedactionFilterTest` in `src/test/java/com/skycryck/tickstatssync/util/LogRedactionFilterTest.java` — attach the filter to a hand-rolled `Logger`, publish `LogRecord`s carrying the token in message + throwable message + args, assert post-filter text scans free of the literal token.
- [X] T011 [P] Define `TickstatsSyncConfig` record in `src/main/java/com/skycryck/tickstatssync/config/TickstatsSyncConfig.java` — flat record with the 13 fields listed in [data-model.md](data-model.md) §Configuration record; hand-rolled `toString()` that redacts `token` ([data-model.md](data-model.md) Lifecycle note).
- [X] T012 Implement `ConfigService` in `src/main/java/com/skycryck/tickstatssync/config/ConfigService.java` — `load()` reads `plugins/TickstatsSync/config.yml` via Paper's `FileConfiguration`, applies the full validation table in [contracts/config-schema.md](contracts/config-schema.md), resolves `TICKSTATSSYNC_GITHUB_TOKEN` fallback ([research.md](research.md) §R11), returns a new immutable `TickstatsSyncConfig` through an `AtomicReference` holder; emits localized English error strings ("config error: …") as specified in [contracts/config-schema.md](contracts/config-schema.md) §Error reporting.
- [X] T013 [P] Create config YAML fixtures under `src/test/resources/config/` — `valid-minimal.yml`, `valid-full.yml`, `invalid-missing-repo.yml`, `invalid-bad-cron.yml` (all per [contracts/config-schema.md](contracts/config-schema.md)).
...
- [X] T015 [P] Add the default `config.yml` resource at `src/main/resources/config.yml` — exact template from [contracts/config-schema.md](contracts/config-schema.md) §Template, including the header-comment PAT guidance (fine-grained first, classic as fallback, dedicated-bot + Write-role warning).
...
- [X] T017 [P] Define `SyncOutcome` enum in `src/main/java/com/skycryck/tickstatssync/sync/SyncOutcome.java` — exactly `SUCCESS_WITH_COMMIT`, `SUCCESS_NO_CHANGES`, `FAILURE` ([data-model.md](data-model.md) §SyncOutcome).
- [X] T018 [P] Define `FailureCategory` enum in `src/main/java/com/skycryck/tickstatssync/sync/FailureCategory.java` — exactly `AUTH`, `NETWORK`, `CONFLICT`, `IO`, `UNKNOWN` ([data-model.md](data-model.md) §FailureCategory; no `CONFIG` value).
- [X] T019 [P] Define `GitOperationException` in `src/main/java/com/skycryck/tickstatssync/git/GitOperationException.java` — checked exception carrying a `FailureCategory category()` field as in [data-model.md](data-model.md) §GitOperationException.
- [X] T020 [P] Implement `SyncMetrics` in `src/main/java/com/skycryck/tickstatssync/sync/SyncMetrics.java` — all atomic fields from [data-model.md](data-model.md) §SyncMetrics (including `AtomicReference<SyncOrchestrator.Trigger> lastTrigger`, written by T033 on every terminal state and read by T041 `/tickstats status`), plus the inner `Reachability { OK, FAILED, UNKNOWN }` enum and a `String configInvalidReason` slot used by inert mode ([plan.md](plan.md) §Bootstrap step 5).
- [X] T021 [P] Implement `SyncLock` in `src/main/java/com/skycryck/tickstatssync/sync/SyncLock.java` — exact shape from [data-model.md](data-model.md) §SyncLock: `tryAcquire()`, `release()`, `heldSince()`, injected `Clock`.
- [X] T022 [P] Author `SyncLockTest` in `src/test/java/com/skycryck/tickstatssync/sync/SyncLockTest.java` — virtual `Clock`, asserts `tryAcquire()` second call returns false while held; `release()` clears holder; `heldSince()` returns the correct `Instant`; FR-003a rejection contract covered.
...
- [X] T044 [P] Add `invalid-malicious-yaml.yml` fixture under `src/test/resources/config/` and extend `ConfigServiceTest` in `src/test/java/com/skycryck/tickstatssync/config/ConfigServiceTest.java` — fixture contains `!!java.net.URL` and similar non-safe tags; assert SnakeYAML 2.x (via Paper) refuses them per [plan.md](plan.md) §Assumptions.
- [X] T045 [P] Add a PAT-scanning end-to-end assertion — new test `src/test/java/com/skycryck/tickstatssync/command/PatLeakageTest.java` (placed in the `command` package so it can reach the deliberately package-private `handleSync`/`handleStatus` entry points without relaxing their visibility) that drives a full success + full failure sync cycle with a well-known canary token, captures every log record published to the plugin logger (including thrown messages and stack traces), every `sendMessage` reply on the fake sender, every plugin-authored commit message, every `/tickstats status` line, and the `TickstatsSyncConfig.toString()` redaction output, and asserts the literal token string never appears in any captured artifact (SC-007, FR-023).
- [X] T046 Ran the quickstart smoke test against a live Paper 26.1.2 build #19-alpha / JDK 25 / Linux server and the disposable repo [Skycryck/tickstats-live-test](https://github.com/Skycryck/tickstats-live-test). Verification record inlined in [quickstart.md](quickstart.md) §7.1 (SC-001, SC-003, SC-004, SC-010 captured; inert-mode recovery verified; R15 log format conformed). New troubleshooting rows added in [quickstart.md](quickstart.md) §10 for malicious-YAML rejection and the Python ≥3.10 requirement for running upstream `generate.py` locally. No dual 1.21.x / 26.x matrix — a single 26.1.2 test run is the sole target ([research.md](research.md) §R16).
- [X] T046a **Tickstats producer-contract verification (SC-002 gate)** — verified empirically via the live-test repo's upstream CI (`.github/workflows/update-stats.yml`, pinned `python-version: "3.12"`): `python scripts/generate.py "stats/${SERVER}/data" --title "${TITLE}"` runs on every push touching `stats/*/data/**` in [Skycryck/tickstats-live-test](https://github.com/Skycryck/tickstats-live-test) and auto-commits the refreshed `stats/serveur-test/index.html` (50 KB) deployed via GitHub Pages; the commit log shows `Auto-update stats dashboard [skip ci]` at each cycle. Local re-run on Python 3.9 was blocked by upstream's PEP 604 type unions (`Path | None`) — fixed the quickstart §8.1 walkthrough to match the real upstream interface (`scripts/generate.py <data_dir_path>`, not `generate.py --stats-dir <stats_root>`; no `requirements.txt` — pure stdlib) and pinned the Python ≥3.10 requirement. The end-to-end producer contract is therefore verified in the exact pipeline operators will use; maintainers MUST re-run §8.1 step 5 locally (with Python ≥3.10) whenever the plugin's output layout changes.
- [X] T047 Ran `./gradlew clean shadowJar` (produced `build/libs/TickstatsSync-1.0.0.jar`, 5256663 bytes ≈ 5.02 MiB). `jar tf` confirms: (a) zero classes survive under `org.eclipse.jgit`, `com.cronutils`, `org.apache.http`, `org.apache.hc`, `org.slf4j`, or `org.bouncycastle`; (b) all runtime-present third-party classes live under `com.skycryck.tickstatssync.shaded.*` — `shaded/jgit` 1705, `shaded/cronutils` 140, `shaded/httpclient` 766, `shaded/slf4j` 60 (`shaded/hc` and `shaded/bc` are defensive relocations; the current dependency graph never pulls Apache HttpClient 5.x nor Bouncy Castle); (c) JSch (`com/jcraft/jsch/`) and both SSH transports (`org/eclipse/jgit/transport/sshd/`, `…/ssh/`) are absent; (d) `META-INF/services/` carries the relocated `com.skycryck.tickstatssync.shaded.slf4j.spi.SLF4JServiceProvider` entry so the bundled NOP binding is discovered under its shaded name.
- [X] T048 [P] Final English-only sweep — scanned `src/`, `README.md`, `config.yml`, `plugin.yml`, and this branch's commit messages against Principle XI in [.specify/memory/constitution.md](.specify/memory/constitution.md). Greps for French accented characters (`[àâäæçéèêëîïôœùûüÿ…]`) and distinctly French vocabulary returned zero matches in the tracked source tree. No drift to correct.
